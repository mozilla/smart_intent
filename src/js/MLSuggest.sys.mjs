...
class _MLSuggest {
   ...
   *     The location extracted from the query, or null if no location is found.
   *   {string} subject
   ...
   */
  async shutdown() {
    const engineKeys = Object.keys(this.#modelEngines);

    for (const key of engineKeys) {
      try {
        const engine = this.#modelEngines[key];
  ...
  async #combineLocations(nerResult, nerThreshold) {
    ...
    return {
      city: cityResult.length ? cityResult.join(" ").trim() : null,
      state: stateResult.length ? stateResult.join(" ").trim() : null
    };
  }
  ...
  #cleanSubject(subject) {
    ...
    if (end) {
      subject = subject
        .substring(0, subject.length - end.length)
        .trimEnd();
    }
    return subject;
  }
